#pragma once
#include <array>
#include <QImage>
#include <QString>
#include <QTemporaryDir>
#include <QThread>
#include <WatermarkCore.hpp>

/*!
 *  \brief  Implementation of the benchmarking background thread
 *  \author Dimitris Karatzas
 */
class BenchmarkWorker : public QThread {
    Q_OBJECT
  public:
    explicit BenchmarkWorker(const int deviceIndex = 0, QObject* parent = nullptr);

  signals:
    // Signals emitted to report benchmark progress, intermediate results, and final score
    void resultReady(const QImage& img, const int p, const float psnr, const double embedTime, const double detectTime, const double embedFps, const double detectFps, const QString& fileName,
        const float correlation);
    void progressUpdated(const int currentStep, const int totalSteps);
    void benchmarkFinished(const double finalEmbedFps, const double finalDetectFps, const int finalScore);
    void benchmarkCanceled();

  protected:
    // Background benchmark loop testing all combinations of p and PSNR
    void run() override;

  private:
    // Sweep values for window size p and target embedding PSNR
    static constexpr std::array<int, 4> pValues{3, 5, 7, 9};
    static constexpr std::array<float, 11> psnrValues{10.0f, 15.0f, 20.0f, 25.0f, 30.0f, 35.0f, 40.0f, 45.0f, 50.0f, 55.0f, 60.0f};

    // Temporary directory containing extracted sample images for benchmark
    QTemporaryDir tempDir;
    QString inputFolder;
    int deviceIndex;
};
