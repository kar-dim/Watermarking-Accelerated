...
// coalesced tiled transpose: column-major uchar to row-major uchar
    ...
    const int height)
{
    ...
    if (col < width && row < height)
        tile[get_local_id(0)][get_local_id(1)] = src[col * height + row];
    barrier(CLK_LOCAL_MEM_FENCE);
    const int outRow = get_group_id(1) * 16 + get_local_id(1);
    const int outCol = get_group_id(0) * 16 + get_local_id(0);
    if (outRow < height && outCol < width)
        dst[outRow * width + outCol] = tile[get_local_id(1)][get_local_id(0)];
}
