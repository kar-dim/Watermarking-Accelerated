    ...
    __local float region[16 + 2][16 + 2]) //hold the 18 x 18 region for this 16 x 16 block
{
    ...
    for (int i = localId; i < 18 * 18; i += get_local_size(0) * get_local_size(1))
    {
        ...
        int globalX =  get_group_id(0) * get_local_size(0) + tileCol - 1;
        int globalY = get_group_id(1) * get_local_size(1) + tileRow - 1;
    ...
    if (x < width && y < height) 
    {
        const int centerCol = get_local_id(0) + 1;
        const int centerRow = get_local_id(1) + 1;
        float dot = 0.0f;
