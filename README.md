...
| p                                 | Window size for masking algorithms. Currently only ```p=3``` is allowed for ```OpenCL```. ```CUDA``` supports ```p=3,5```. ```Eigen``` implementation supports values of ```p=3,5,7``` and ```9```. |
